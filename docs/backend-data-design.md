...
更新：2026-09-09。状态：正式模型的工程设计草案；2026-09-11 已按此设计建立 V2 实验模型并完成数据库验证，见 [V2 报告](backend-v2-report.md)。本逻辑设计不依赖 ORM。依据：[业务规格](server-alpha-spec.md)、[架构设计](backend-architecture.md)。字段命名为建议，不代表接口已定稿。
...
ORM 决策见 [比较记录](orm-evaluation.md)。认证细节、图片上传、计价币种及部署目标仍待对应阶段确认；本文件不构成这些产品选择的批准，数据库实验结果与仍待认证/API 阶段的验收见 V2 报告。
