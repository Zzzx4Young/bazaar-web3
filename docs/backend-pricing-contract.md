...
每个不可变快照保存 id、provider、fetchedAt、expiresAt 和全白名单汇率。Coinbase 快照有效期 1 小时；第一笔排序请求复用有效快照，否则在数据库事务外抓取并保存，进程内合并并发刷新。快照保存在数据库，其他需要参考汇率的服务可复用同一 `RatesService`，不需要各页面分别抓取供应商。固定 HTTPS 地址、3 秒超时、最大 128 KiB 响应，不跟随重定向；验证响应基准为 USD、全部白名单值为有限正十进制字符串。缺失任一支持币种时拒绝新快照，不把缺失价格当 0。

该供应商响应不提供每个币种的行情发生时间，因此 fetchedAt 仅表示本服务获取时间，sourceAsOf 返回 null；不能标成“实时成交价”。1 小时 TTL 限制本服务缓存年龄，不保证上游行情延迟。它只用于参考排序，不改变模拟结算金额。
