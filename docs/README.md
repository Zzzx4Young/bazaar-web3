...
更新：2026-09-16。I1—I8 与 M1 Alpha 发布加固已完成；提交 `113c1ee` 的远端 CI 两个 job
均成功。内部 Alpha 的认证、
商品、订单、交付、退款恢复、
前端联调和受控内测已经完成。当前工作区已将默认页面商品、卖家、个人中心和通知从旧 Demo
数据切换到服务端事实或明确的未开放状态。当前状态以执行计划
和阶段记录为准。
...
| [前端功能基线](frontend-prototype-roadmap.md) | 历史前端原型能力；当前 Alpha 页面以源码和阶段记录为准 |
| [前端技术与启动](frontend-stack-recommendation.md) | 实际技术栈、启动及检查方式 |
| [演示数据契约](mock-data-spec.md) | 已被 Alpha 默认页面取代的历史 Mock 和浏览器存储格式 |
