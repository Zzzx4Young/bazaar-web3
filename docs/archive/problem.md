...
本节覆盖下方历史分析中的旧判断。下方原文保留用于追溯，不能直接执行其中的推荐方案或估时；当前决策入口为 [蓝图复核](../review-corrections.md)。

| ID | 当前判断 | 状态与后续 |
|---|---|---|
| P1 | 撤回“2-of-2 是单签延迟/拆分一把密钥”：独立密钥仍有签名门槛，但同人保管缺少人员独立性；丢失一把可能不可用 | 人员、阈值、恢复和模块权限待设计，不能声称某方案消除全部 P0 |
| P2 | T-130 已撤回未交付超时给卖家放款；“自动释放”不能解释为退款 | 与 P3/P5 一起确定退款、放款的状态与期限 |
| P3 | 已交付声明不等于已完成验收；必须保留争议窗口和终态规则 | 待统一状态表，撤回旧表的偏袒推断及估时 |
| P4 | T-230 已改 digital/<listingId>/<fileId>，并要求订单关联固定文件版本；订单并非必须付款后才创建 | 原命名文档矛盾已修正，尚未实现；下载时点另见 P5 |
| P5 | 撤回“RELEASED 后才下载、永久保留、放款后拒绝交付仍可争议”的建议，后者与不可逆终态矛盾 | 交付、验收、保留期限、访问权限及故障救济待决定 |
| P6 | 多币种不是仅加一个参数，需要精度、金额单位、代币行为和测试边界 | 按选定阶段定范围，旧增量估时无效 |
| P7 | 测试网需要测试代币及 Gas 的用户引导；网页提示不能代替可用性验收 | 仅测试网阶段适用 |
| P8 | 应用本地可控时间环境测超时，再在公开测试网验证部署与交易流程 | 不要求 E2E 等数天，也不能把 Solidity vm.warp 直接当浏览器/RPC API |
| P9 | 邮箱后台权限与 Safe 链上签名权限分离；仅邮箱失陷不等于具备 Safe 阈值签名 | 后台认证、提案、独立签名与审计路径待设计 |
| P10 | 当前尚无钱包集成，不能把历史风险当已存在的缺陷 | 接入后验收 SSR 与恢复，无依据预先更换 Router |

技术依据与源文档对应见复核记录。以上是事实纠正与待决策边界，不代表已批准具体实现。

## 历史记录（2026-07-31，以下内容不再作为决策建议）
