...
> **2026-09-07 示例复核**：以下代码未经编译、测试或审计，不是可部署合约。已发现卖家可传零秒放款期限、escrow 缺少所声称的 pause 实现、Factory 仲裁调用路径未完整展示等问题。文中的“自动”仍需有人提交交易，不能视为链上定时器。仅保留概念参考，实施前按新规则重设计，见 [R04/R05](../review-corrections.md)。
