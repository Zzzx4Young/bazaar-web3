...
商品和收藏挂载后读取 localStorage，同页通过自定义事件同步，其他标签页通过 `storage` 事件同步；通知后重新读取并校验商品缓存，不信任事件携带的数据。订单保留 Zustand persist 的恢复与数据格式，创建、状态更新、重置先成功写入存储再更新内存；订单跨标签页实时同步尚未实现。

刷新不会主动清理数据；配额满也不自动清空。发布存储失败会显示错误；收藏失败不假装成功。订单保存失败保留原内存与磁盘数据，购买弹窗回到确认步骤提示错误，允许恢复空间或权限后重试。用户清除站点数据后，本地新增商品、收藏和订单消失，静态 Mock 仍存在。聊天仅存组件内存，重载后消失。

本地数据属于当前浏览器和源，不跨设备同步。本地商品详情链接在其他浏览器不可用。非法 JSON/非数组的商品收藏缓存回退为空。商品缓存通过 `src/lib/item-cache.ts` 逐条校验字段，忽略不合法元素、保留有效元素，兼容缺少 primaryCategory 的旧商品；读取时不改写原始缓存。后续成功发布会保存恢复后的有效列表与新商品，无效条目不继续写入。此校验不代表收藏和订单已有完整 schema 迁移，也不验证外部媒体可用性。
